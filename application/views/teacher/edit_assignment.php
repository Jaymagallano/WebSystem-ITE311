    ...
    <div class="col-md-8 mx-auto">
        <div class="card">
            ...
            <div class="card-body">
                ...
                <?= form_open('teacher/edit_assignment/' . ($assignment->id ?? '')) ?>
                <div class="mb-3">
                    <label for="course_id" class="form-label">Course</label>
                    <select class="form-select" id="course_id" name="course_id" disabled>
                        <option value="<?= $assignment->course_id ?? '' ?>"><?= ($assignment->course_code ?? '') ?> -
                            <?= ($assignment->course_title ?? '') ?></option>
                    </select>
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control" id="title" name="title"
                        value="<?= set_value('title', $assignment->title ?? '') ?>" required>
                </div>

                <div class="mb-3">
                    <label for="description" class="form-label">Description *</label>
                    <textarea class="form-control" id="description" name="description" rows="5"
                        required><?= set_value('description', $assignment->description ?? '') ?></textarea>
                </div>
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <input type="datetime-local" class="form-control" id="due_date" name="due_date"
                                value="<?= set_value('due_date', date('Y-m-d\TH:i', strtotime($assignment->due_date ?? ''))) ?>"
                                required>
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <input type="number" class="form-control" id="max_points" name="max_points"
                                value="<?= set_value('max_points', $assignment->max_points ?? '') ?>" min="1" required>
                        </div>
                    </div>
                </div>
